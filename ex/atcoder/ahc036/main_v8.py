...
class PathsStats:
    ...
    def get_paths_in_most_visited_order(self):
        paths = []
        for item in sorted(self.paths_visited_specific_times.items(), key=lambda item: item[0], reverse=True):
            for path in item[1]:
                paths.extend([path])
        return paths
...
def create_A_along_most_visited_cities(G, start_city, La, paths_stats, coordinate):
    ...
    paths_most_visited_order = paths_stats.get_paths_in_most_visited_order()
    for path in paths_most_visited_order:
        if len(A) + len(path) >= La:
            break
        for index in get_indices_of_the_city_in_A(path[0], A):
            if index-1 >= 0 and A[index-1] == path[1]:
                continue
            if index+1 < len(A) and A[index+1] == path[1]:
                continue
...
def get_path_to_dst_by_B(G, start, dst, B):
    if dst not in B:
        return None

    start_adjs = G[start]
    #logger.debug(f"B:          {B}")
    #logger.debug(f"start_adjs: {start_adjs}")
    #logger.debug(f"dst:        {dst}")

    def left_recurse(current, i, path):
        if i < 0:
            return False
        if B[i] not in G[current]:
            return False
        path.append(B[i])
        if B[i] in start_adjs:
            return True
        return left_recurse(B[i], i-1, path)

    def right_recurse(current, i, path):
        if i >= len(B):
            return False
        if B[i] not in G[current]:
            return False
        ...
        if B[i] in start_adjs:
            return True
        return right_recurse(B[i], i+1, path)

    path = [ dst ]
    if left_recurse(dst, B.index(dst) - 1, path):
        path = list(reversed(path))
        #logger.debug(f"found path: {path}")
        return path
    path = [ dst ]
    if right_recurse(dst, B.index(dst) + 1, path):
        path = list(reversed(path))
        #logger.debug(f"found path: {path}")
        return path
    ...
    for xth, path in enumerate(paths):
        ...
        for dst_city in path:
            if dst_city in G[current]:
                ...
                path = get_path_to_dst_by_B(G, current, dst_city, recorder.get_B())
                if path:
                    recorder.move_by_path(path)
                elif set(shortcut_path) in set(recorder.get_B()):
